...
with open("stock_basics.csv") as csvfile:
    ...
    for row in reader:
        stockcode = row['code']
        # 日数据
        df = ts.get_k_data(stockcode, ktype='D', start='2017-09-21',end='2017-09-22',retry_count =5,pause =1)
        df["code"] = pd.Series(stockcode, index=df.index)

        df.to_sql('k_day_data', engine, if_exists='append')
        # 历史数据
        df = ts.get_hist_data(stockcode, start='2017-09-21',end='2017-09-22',retry_count =5,pause =1)
        df["code"] = pd.Series(stockcode, index=df.index)
